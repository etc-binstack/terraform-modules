...
locals {
  primary_key_policy_vars = var.enable_module ? {
    region              = var.region
    ...
    replica_kms_key_arn = var.enable_multi_region ? aws_kms_replica_key.replica_key[0].arn : aws_kms_key.primary_key[0].arn
    secondary_region    = var.enable_multi_region ? var.secondary_region : var.region
  } : {}
}
...
locals {
  replica_key_policy_vars = var.enable_module && var.enable_multi_region ? {
    region              = var.secondary_region
    account_id          = local.account_id
    lambda_role_name    = aws_iam_role.lambda_role[0].name
    kms_key_arn         = aws_kms_replica_key.replica_key[0].arn
    replica_kms_key_arn = aws_kms_key.primary_key[0].arn
